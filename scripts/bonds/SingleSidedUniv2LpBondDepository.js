...
async function main() {
    ...
    const bond = await Bond.deploy(fhmAddress, usdbAddress, reserve.address, treasury.address, daoAddress, usdbMinterAddress, balancerVaultAddress, usdbDaiLpAddress, masterChefAddress, usdcPriceFeedAddress);
    await bond.deployed();
    ...
    console.log(`\nVerify:\nnpx hardhat verify --network ${network} `+
        `${bond.address} "${fhmAddress}" "${usdbAddress}" "${reserve.address}" "${treasuryAddress}" "${daoAddress}" "${usdbMinterAddress}" "${balancerVaultAddress}" "${usdbDaiLpAddress}" "${masterChefAddress}" "${usdcPriceFeedAddress}"`);
}
