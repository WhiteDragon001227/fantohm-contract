...
async function main() {
	...
	for (let i = 0; i < reserves.length; i++) {
		...
		const bond = await Bond.deploy( fhmAddress, sfhmAddress, reserve.address, treasury.address, daoAddress, reserve.feed, fhudMinterAddress, fhmCirculatingSupply);
		...
		// Set bond terms
		await bond.initializeBondTerms(reserve.bondBCV, bondVestingLength, minBondPrice, maxDiscount, maxBondPayout, maxBondDebt, intialBondDebt);
