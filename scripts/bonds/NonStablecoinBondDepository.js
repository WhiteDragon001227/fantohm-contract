...
async function main() {
	...
	for (let i = 0; i < reserves.length; i++) {
		...
		// Deploy Bond
		const Bond = await ethers.getContractFactory('NonStablecoinBondDepository');
		const bond = await Bond.deploy( fhmAddress, reserve.address, treasury.address, daoAddress, reserve.feed, fhudMinterAddress, fhmCirculatingSupply);
