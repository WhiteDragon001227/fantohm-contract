...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. FHMs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that FHMs distribution occurs.
        uint256 accFhmPerShare;   // Accumulated FHMs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The FHM TOKEN!
    FantohmERC20Token public fhm;
    // Dev address.
    address public devaddr;
    // LQDR tokens created per block.
    uint256 public fhmPerBlock;
    ...
    event UpdateEmissionRate(address indexed user, uint256 fhmPerBlock);

    constructor(
        FantohmERC20Token _fhm,
        address _devaddr,
        address _feeAddress,
        uint256 _fhmPerBlock,
        uint256 _startBlock
    ) public {
        fhm = _fhm;
        devaddr = _devaddr;
    ...
    function pendingLqdr(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accLqdrPerShare = pool.accFhmPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 fhmReward = multiplier.mul(fhmPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accLqdrPerShare = accLqdrPerShare.add(fhmReward.mul(1e12).div(lpSupply));
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accFhmPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function safeFhmTransfer(address _to, uint256 _amount) internal {
        uint256 fhmBal = fhm.balanceOf(address(this));
    ...
    function updateEmissionRate(uint256 _fhmPerBlock) public onlyOwner {
        massUpdatePools();
        fhmPerBlock = _fhmPerBlock;
