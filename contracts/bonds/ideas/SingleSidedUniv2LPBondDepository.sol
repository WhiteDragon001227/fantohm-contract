...
contract SingleSidedUniv2LPBondDepository is Ownable, ReentrancyGuard {
    ...
    event BondRedeemed(address indexed recipient, uint payoutInPrinciple, uint amountInLP, uint remainingInPrinciple);
    event BondIlProtectionRedeem(address indexed recipient, uint payoutInFhm, uint payoutInPrinciple);
    ...
    function deposit(
        ...
        // must be > 0.01 stable ( underflow protection )
        ...
        uint _lpTokenAmount = createLP(_amount, _amount);
    ...
    function ilProtectionClaimable(address _recipient, uint _lpTokenAmount, uint _principleAmount) public view returns (uint) {
        ...
        return ilInPrinciple.mul(uint(assetPrice())).div(1e8); // 8 decimals feed
    }
