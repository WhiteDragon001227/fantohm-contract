...
contract FHM_UsdbBondStakingDepository is Ownable {
    ...
    address public immutable USDB; // token used to create bond
    address public immutable treasury; // mints FHM when receives principle
    address public immutable DAO; // receives profit share from bond
    address public immutable usdbMinter; // minting USDB for burning FHM
    ...
    *  @notice set contract for auto stake
    ...
    function depositFHM(
        ...
        // 2. mint USDB based on market price
        uint price = IUsdbMinter(usdbMinter).getMarketPrice();
        uint stableCoinAmount = _amount.mul( price.div( 10**2 ) );

        IERC20( FHM ).approve( usdbMinter, _amount );
        IUsdbMinter(usdbMinter).mint( stableCoinAmount, price );

        // 3. use USDB to deposit to long term bond
    ...
    function deposit(
        ...
        uint value = ITreasury( treasury ).valueOf( USDB, _amount );
         ...
         */
        IERC20(USDB).approve( address( treasury ), _amount );
        ITreasury( treasury ).deposit( _amount, USDB, profit );
